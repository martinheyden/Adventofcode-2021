...
enum Coordinate {
    TwoDim {depth: i64, position:i64},
    ThreeDim {depth:i64, angle:i64, position:i64}
}
...
impl Coordinate {
    fn forward(&mut self, arg:i64) {
        match self {
            ...
            Coordinate::ThreeDim{depth, angle, position} =>{ *position +=arg;
                *depth += arg* *angle;},
        };
    }
    ...
    fn product (& self) -> i64 {
        match self {
            ...
            Coordinate::ThreeDim{depth, position, ..} => *depth* *position,
        }
    }
}
...
fn problem_a(filename: &str) ->i64 {
    navigate(filename,Coordinate::TwoDim{depth: 0, position:0})
}


fn problem_b(filename: &str) ->i64 {
    navigate(filename,Coordinate::ThreeDim{depth: 0, angle:0, position:0})
}
